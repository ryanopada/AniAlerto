...
try {
    ...
    $tStmt = $db->prepare("
        SELECT mt.*, fb.name AS batch_name, fb.municipality AS batch_municipality
    ...
    foreach ($templates as $template) {
        ...
        foreach ($workers as $worker) {
            ...
            // Check if the task is currently in Weather_Hold or delayed to the future.
            if ($taskId) {
                $holdCheck = $db->prepare("SELECT status, due_date FROM scheduled_tasks WHERE id = :id");
                $holdCheck->execute([':id' => $taskId]);
                $taskRow = $holdCheck->fetch(PDO::FETCH_ASSOC);
                
                if ($taskRow) {

                    if ($taskRow['due_date'] > $nowDate) {
                        $results['details'][] = "Skip (Delayed to {$taskRow['due_date']}): {$worker['name']} / template #{$templateId}";
                        continue;
                    }
                }
            }
        ...
        // Mark template as queued so it is not processed again on subsequent cron runs
        $updateTemplateStmt = $db->prepare("UPDATE message_templates SET queued_at = NOW() WHERE id = :id");
        $updateTemplateStmt->execute([':id' => $templateId]);
    }
