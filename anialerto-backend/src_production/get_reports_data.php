...
$workerMonitoringRows = safeRows($conn, "
    SELECT w.id, w.name, w.phone,
           ...
           SUM(CASE WHEN sl.response_text LIKE 'DELAY%' THEN 1 ELSE 0 END) as delay_count,
           SUM(CASE WHEN sl.response_text LIKE 'HELP%' OR sl.response_text IN ('PEST', 'UOD') THEN 1 ELSE 0 END) as help_count,
...
$upcomingActivities = safeRows($conn, "
    ...
    WHERE mt.active = 1 AND mt.scheduled_send_datetime >= CURDATE() AND $upcoming_where
...
$pestAlerts = safeRows($conn, "
    SELECT sl.id, w.name as workerName, sl.phone, 
           CASE 
               WHEN sl.response_text LIKE 'HELP_CUSTOM:%' THEN 'HELP_CUSTOM'
               WHEN sl.response_text LIKE 'HELP:%' THEN 'HELP'
               WHEN sl.response_text LIKE 'DELAY:%' THEN 'DELAY'
               ELSE sl.response_text 
           END as alertType,
           sl.response_text as details,
           sl.received_at, sl.created_at
    FROM sms_logs sl
    LEFT JOIN workers w ON sl.worker_id = w.id
    WHERE (sl.response_text IN ('HELP', 'PEST', 'UOD') OR sl.response_text LIKE 'HELP:%' OR sl.response_text LIKE 'HELP_CUSTOM:%' OR sl.response_text LIKE 'DELAY:%') AND $log_where
...
$advisoryRows = safeRows($conn, "
    SELECT
        ...
        SUM(CASE WHEN response_text LIKE 'DELAY%' THEN 1 ELSE 0 END) as total_delayed
    FROM sms_logs sl
...
$delayEvents = safeRows($conn, "
    SELECT w.name AS workerName, fb.name AS batchName, mt.name AS taskName, 
           CASE WHEN sl.response_text LIKE 'DELAY: %' THEN SUBSTRING(sl.response_text, 8) ELSE sl.response_text END AS reason, 
           sl.received_at AS timestamp
    FROM sms_logs sl
    LEFT JOIN workers w ON sl.worker_id = w.id
    ...
    WHERE sl.response_text LIKE 'DELAY%' AND sl.direction = 'Outbound' AND $log_where
...
$helpEvents = safeRows($conn, "
    SELECT w.name AS workerName, fb.name AS batchName, mt.name AS taskName, 
           CASE WHEN sl.response_text LIKE 'HELP: %' THEN SUBSTRING(sl.response_text, 7) ELSE sl.response_text END AS category, 
           sl.received_at AS timestamp
    FROM sms_logs sl
    LEFT JOIN workers w ON sl.worker_id = w.id
    LEFT JOIN scheduled_tasks st ON sl.task_id = st.id
    LEFT JOIN farm_batches fb ON st.batch_id = fb.id
    LEFT JOIN message_templates mt ON st.template_id = mt.id
    WHERE (sl.response_text LIKE 'HELP%' OR sl.response_text LIKE 'HELP_CUSTOM%') AND sl.direction = 'Outbound' AND $log_where
    ORDER BY sl.received_at DESC
    LIMIT 100
");
