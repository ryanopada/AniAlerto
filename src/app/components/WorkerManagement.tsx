      ...
      <Card className="shadow-sm border-none">
        <CardHeader className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <CardDescription>Manage your field team assignments</CardDescription>
          </div>
        ...
        <CardContent>
          {loading ? (
            <div className="flex justify-center p-8"><Loader2 className="animate-spin text-[#8acb88]" /></div>
          ) : (
            <Table>
              ...
              <TableBody>
                {filteredWorkers.map((worker) => (
                  <TableRow key={worker.id}>
                    ...
                    <TableCell><Badge variant="outline">{worker.assignedBatch}</Badge></TableCell>
                    <TableCell>
                      <Badge className={worker.status === "Active" ? "bg-[#e4fde1] text-[#648381]" : "bg-gray-100 text-gray-500"}>
                        {worker.status}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      <Button variant="ghost" size="sm" onClick={() => handleOpenEdit(worker)}><Edit className="h-4 w-4" /></Button>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 pt-4">
            ...
            <div className="space-y-2">
              <Label>Status</Label>
              <select className="w-full border rounded-md p-2" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value as any})}>
                <option value="Active">Active</option>
                <option value="Inactive">Inactive</option>
              </select>
            </div>
            <Button type="submit" className="w-full bg-[#8acb88] hover:bg-[#648381]">Save Worker</Button>
          </form>
        </DialogContent>
      </Dialog>
    </div>
